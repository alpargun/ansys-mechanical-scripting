...
# ==========================================
# --- CONFIGURATION (FULL 64-CASE BATCH) ---
# ==========================================
MIN_PRESSURE = 1       
MAX_PRESSURE = 100000  
STEP_SIZE = 25000      
...
def setup_analysis_steps(analysis):
    print("--- Configuring Analysis Steps for Fast Extreme Bending ---")
    ...
    for step in [1, 2]:
        ...
        # SPEED OPTIMIZATION: Unthrottled substeps to allow massive leaps
        settings.SetInitialSubsteps(step, 20)  
...
def set_load_schedule(load_obj, peak_val):
    ...
    pressures = [Quantity(str(MIN_PRESSURE) + " [Pa]"), Quantity(str(peak_val) + " [Pa]"), Quantity(str(MIN_PRESSURE) + " [Pa]")]
...
def export_pressure_profile(case_folder, base_name, load_p1, load_p2, load_p3):
    """Creates a lightweight CSV mapping Time to Instantaneous Pressure (in kPa)."""
    file_path = os.path.join(case_folder, base_name + "_PressureProfile.csv")
    time_steps = [0.0] + [round((i + 1) * (2.0 / VIDEO_FRAMES), 4) for i in range(VIDEO_FRAMES)]
...
def export_consolidated_data(case_folder, base_name, mesh_data, load_p1, load_p2, load_p3, def_x, def_y, def_z, strain, solution_obj):
    """Exports Node spatial data and stamps the file with the peak target pressures."""
...
# --- MAIN EXECUTION ---
# ==========================================
print("--- Starting 64-Case Video-Focused Hysteresis Dataset ---")
...
if not (p1 and p2 and p3 and total_def):
    ...
    # Generate perfect intervals [1, 25000, 50000, 75000, 100000]
