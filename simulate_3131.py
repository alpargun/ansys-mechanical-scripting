...
# ==========================================
# --- CONFIGURATION (FIXED 8s PROFILE) ---
# ==========================================
...
# Fixed Timings for ALL cases (Ensures perfectly uniform ML Tensors)
T_UP = 3.0       # 3-second ramp up
T_HOLD = 4.0     # 1-second hold (Ends at 4s)
T_DOWN = 7.0     # 3-second ramp down (Ends at 7s)
DURATION = 8.0   # 1-second final pause (Total video length)
FPS = 30
VIDEO_FRAMES = int(DURATION * FPS) # Always yields exactly 240 frames
...
main_output_folder = os.path.join(base_folder, "Run_8s_Profile_" + timestamp)
...
def setup_analysis_steps(analysis):
    """Configures the Newton-Raphson solver dynamically for the 3 distinct phases."""
    print("--- Configuring Analysis Steps for Fixed Duration Profile ---")
    settings = analysis.AnalysisSettings
    
    # 3 distinct steps match the inflation, the hold, and the deflation + pause
    settings.NumberOfSteps = 3
    settings.SetStepEndTime(1, Quantity(str(T_UP) + " [s]"))    # Phase 1: Ramp Up
    settings.SetStepEndTime(2, Quantity(str(T_HOLD) + " [s]"))  # Phase 2: Static Hold
    settings.SetStepEndTime(3, Quantity(str(DURATION) + " [s]"))# Phase 3: Ramp Down & Pause
...
def set_load_schedule(load_obj, peak_val):
    """Injects the defined pressure schedule into the Ansys Tabular Data array."""
    times = [
        Quantity("0 [s]"), 
        Quantity(str(T_UP) + " [s]"), 
        Quantity(str(T_HOLD) + " [s]"), 
        Quantity(str(T_DOWN) + " [s]"),
        Quantity(str(DURATION) + " [s]")
    ]
    pressures = [
...
# --- MAIN EXECUTION ---
# ==========================================
print("--- Starting Video-Focused Hysteresis Dataset Recovery (125 Cases) ---")
...
if not (p1 and p2 and p3 and total_def):
    ...
    # Generate the grid array (0k, 25k, 50k, 75k, 100k)
    ...
    load_cases = []
    # Generates exactly 125 combinations (5 * 5 * 5)
    ...
    for i, case in enumerate(load_cases):
        ...
        try:
            print("\n=== Processing Case {}/125 [Peak: P1={}, P2={}, P3={}] ===".format(case_num, val_p1, val_p2, val_p3))
