...
# ==========================================
# --- CONFIGURATION (10s at 30 FPS) ---
# ==========================================
MIN_PRESSURE = 1       # 1 Pa absolute minimum
DURATION = 10.0        # 10 second simulations
FPS = 30
VIDEO_FRAMES = int(DURATION * FPS) # 10s * 30 FPS = 300 Frames
GROWTH_FACTOR = 2.0    
...
# Define the 5 new random action sequences
run_sequence = [
    {"filename": "Safe_Random_Actions_1.csv", "label": "Random_1"},
    {"filename": "Safe_Random_Actions_2.csv", "label": "Random_2"},
    {"filename": "Safe_Random_Actions_3.csv", "label": "Random_3"},
    {"filename": "Safe_Random_Actions_4.csv", "label": "Random_4"},
    {"filename": "Safe_Random_Actions_5.csv", "label": "Random_5"}
]
...
def setup_analysis_steps(analysis):
    ...
    # TIGHTENED AUTO-STEPPING FOR 10s FAST DYNAMICS
    settings.SetAutomaticTimeStepping(1, AutomaticTimeStepping.On)
    settings.SetInitialSubsteps(1, 1000)   # 100 Hz Physics Resolution for 10s
    settings.SetMinimumSubsteps(1, 1000)   # NEVER drop below 100 Hz
    settings.SetMaximumSubsteps(1, 10000)  # 1000 Hz deep bisection for resonance
    print("    Dynamic Auto-Stepping configured for 10-second fast solve.")
...
def blocking_solve(analysis_obj, solution_obj):
    ...
    print("      [Solver]: Starting Fast 10-Second Iterative Solve...")
    analysis_obj.Solve()
...
# ==========================================
print("--- Starting Safe Random Actions Batch Run ---")
...
if not (p1 and p2 and p3 and total_def):
    ...
    for run in run_sequence:
        ...
        print("\n==================================================")
        print(">>> NOW PROCESSING: {} ({})".format(csv_filename, run_label))
        print("==================================================")
        ...
        folder_name = "Run_Safe_" + run_label + "_30FPS_" + batch_timestamp
        ...
        if not success:
            print("      !!! SOLVE FAILED FOR {}: {} !!!".format(run_label, msg))
            continue 
            
        # 3. Export Results
        base_name = "Safe_" + run_label + "_30FPS_10s"
        
        print("      Exporting 30 FPS Pressure Profile...")
        export_30fps_pressure_profile(main_output_folder, base_name, p1, p2, p3)

        print("      Exporting Tip Displacement CSV ({} Frames)...".format(VIDEO_FRAMES))
        ...
        print(">>> FINISHED: {} pipeline successfully completed.".format(run_label))
...
print("\nAll Safe Random batch runs complete!")